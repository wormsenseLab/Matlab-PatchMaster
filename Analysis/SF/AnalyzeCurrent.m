function [AvgMaxCurrent,AvgMaxCurrentMinus,AvgMaxCurrentOff,AvgMaxCurrentMinusOff,Start,StartOffBelow,Ende,EndeOff,ASubtractAvg,LengthRamp,LengthInms] = AnalyzeCurrent(isFiveStep,isStep,ActuSensor,StartBase,Aall,ASubtract,fs,SlopeActu,BelowPlateau,CellMaxActuFirst,interval); %tf,isStep, SteigungAbs,
...
EndeOff(i) = StartOffBelow(i) + (fs/50); % currently not in use
