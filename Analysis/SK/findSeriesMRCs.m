% findSeriesMRCs.m
%
% For finding MRCs in a single series, not after taking the mean trace per
% grouping. Can be used in conjunction with newStepFind.m for getting
% "seriesStimuli" variable.

function cellPeaks = findSeriesMRCs(seriesStimuli, traceData, sf, dataType)
...
for iSweep = 1:nSweeps
    ...
    cellPeaks(iSweep,1) = pk;
    cellPeaks(iSweep,2) = pkLoc;
    cellPeaks(iSweep,3) = tau;
    cellPeaks(iSweep,4) = pkFit;
    
end

cellPeaks(:,5) = seriesStimuli(:,6);
cellPeaks(:,6) = seriesStimuli(:,7);
end